...
public class DelegateTypewriter : MonoBehaviour
{
    ...
    void Start()
    {
        delay = DelegateSettings.TEXT_DELAY;                 // Initialize value of delay
        DelegateSettings.onTextDelayChanged += UpdateDelay;  // Subscribe for changes to text delay
    }

    void OnDestroy()
    {
        DelegateSettings.onTextDelayChanged -= UpdateDelay;  // Must unsubscribe when being destroyed
    }
