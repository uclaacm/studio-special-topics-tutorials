...
public class FinalTypewriter : MonoBehaviour
{
    ...
    void Start()
    {
        delay = FinalSettings.TEXT_DELAY;                           // Initialize value of delay
        FinalSettings.onTextDelayChanged.AddListener(UpdateDelay);  // Subscribe for changes to text delay
    }

    void OnDestroy()
    {
        FinalSettings.onTextDelayChanged.RemoveListener(UpdateDelay);   // Must unsubscribe when being destroyed
    }
